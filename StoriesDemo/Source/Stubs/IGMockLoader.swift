...
struct IGMockLoader {
    ...
    static func loadMockFile(named fileName:String,bundle:Bundle = .main) throws -> UserStories {
        ...
        do {
            ...
            if let _ = try JSONSerialization.jsonObject(with: data as Data, options: .allowFragments) as? [String:Any] {
                let stories = try JSONDecoder().decode(UserStories.self, from: data)
                return stories
            }else {
    ...
    static func loadAPIResponse(response: [String: Any]) throws -> UserStories {
